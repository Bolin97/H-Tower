...
class CONVERSATION:
    def __init__(self):
        ...
        self.token = []  # 句子涉及的token
        self.value = []  # 句子涉及的value
...
# 这里面的token一定是用户有需求的，有的完成了，有的没完成
class PROCESSINGTOKEN:
    def __init__(self):
        self.infer_value = None  # 当前的infer_value
        self.value_type = None  # value的类型(选项或数值)
        self.dialog_history = []  # 关于这个value的对话历史
        self.options = []  # 这个value的可选项
        self.unsuccessful = 0  # 推理失败的次数
        self.interested = None  # 用户此次对话是否提到该token
