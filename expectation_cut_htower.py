...
def filter_and_save_conversations(input_filename, output_filename, zz,usertypedata):
    """
    ...
    for record in data:
        ...
        for i, turn in enumerate(conversations):
            ...
            if turn.get("role") == "user":
                ...
                prospect = turn.get(usertypedata)
                trigger_index = i
        ...
        if trigger_index != -1:
            ...
            for i, t in enumerate(truncated_conversations):
                ...
                # --- 处理代理轮次 ---
                    ...
                    target_features_agent = t.get("feature_value")
                    ...
                    if t.get('s_type') == "profile_based_determination":
                        ...
                        if isinstance(profile_values, dict):
                            ...
                            if profile_values.values() is not None:
                                ...
                                agent_keys.update(profile_values.keys())
...
# --- 配置 ---
INPUT_FILE = "data/conversations.json" # 输入文件路径
OUTPUT_FILE = "expect0_85.json"                  # 输出文件路径
PROSPECT_THRESHOLD = 0.3                     # 设置用户预期阈值 (termination_value)
UER_TYPE_DATA='prospect'                        #用户类型指标选择:1.期望用户:'prospect';2.容忍度用户:"tolerance"
# --- 运行处理流程 ---
filter_and_save_conversations(INPUT_FILE, OUTPUT_FILE, PROSPECT_THRESHOLD,UER_TYPE_DATA)